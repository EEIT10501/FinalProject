					...
					<c:choose>
							...
							<c:otherwise>
								...
								<c:forEach var="applicant" items="${applicantsByJob}" varStatus="loop">
									<c:if test="${applicant.appliedStatus =='待回應'}">
										<div class="row">
										<div class="col-sm-12">
						...
						<!-------------------------------最底下的應徵狀態------------------------------------------>
						<div class="row" style="background-color: #f0f0f0; text-align: center; height: auto">
							<br>
							...
							<!------------------------彈跳出來的邀約確認視窗始----------------------------------------------------->
								...
								<!-------------------------------最底下的應徵狀態終------------------------------------------>
									...
									<c:if test="${(totalApplicants > 0) or (invitedCheck > 0) }">
									   <b>共用 ${totalApplicants + invitedCheck} 筆應徵紀錄: </b><p></p>
									</c:if>
									...
									<c:if test="${invitedCheck > 0}">
										...
										可前往
										<a href="${pageContext.request.contextPath}/applicationNInterview">
										邀約管理										
										</a>查看更新結果。
									</c:if>
