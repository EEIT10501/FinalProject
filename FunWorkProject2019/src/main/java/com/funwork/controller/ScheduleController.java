...
//			System.out.println(starttime);
      ...
      float restTime = (float) ((Timestamp.valueOf(endtime).getTime() - Timestamp.valueOf(starttime).getTime())
          / (1000 * 60 * 60) / 4 * 0.5);
      schedule.setRestHour(restTime);

      Interview interview = interviewService.findByAdmitJobUserName(jobId, (String) jsonObject.get("scheduleName"));
...
//			System.out.println("staffSchedules size:"+staffSchedules.size());
  ...
  public String selectWageStaff(Model model, @RequestParam("start") String start, @RequestParam("end") String end,
    ...
    if (loginUser != null) {
      ...
      List<Schedule> staffScheduleList = scheduleService.getSchedulesByNameAndTime(loginUser.getUserName(), startTime,
          endTime);
...
//				object.put("color", sj.getColor());
  ...
  public String userCalendarDate(Model model, HttpServletRequest request, @PathVariable("time1") String time1,
      @PathVariable("time2") String time2) {
    ...
    if (user != null) {
      List<Schedule> scheduleList = scheduleService.getUserScheduleByRange(user.getUserId(), sqldate.valueOf(time1),
          sqldate.valueOf(time2));
      JSONArray jsonArray = new JSONArray();
      JSONObject object = null;
      for (Schedule sj : scheduleList) {
        object = new JSONObject();
...
//				object.put("color", sj.getColor());
  ...
  public String jobCalendarDate(Model model, @PathVariable("jobId") Integer jobId, @PathVariable("time1") String time1,
      @PathVariable("time2") String time2) {
    java.sql.Date sqldate = new java.sql.Date(0);

    List<Interview> interviewList = interviewService.findInterviewByAdmit(jobId);

    List<Schedule> scheduleList = scheduleService.getJobSchedulesByRange(jobId, sqldate.valueOf(time1),
        sqldate.valueOf(time2));
    JSONArray jsonArray = new JSONArray();
    JSONObject object = null;
    for (Schedule sj : scheduleList) {
